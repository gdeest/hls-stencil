...
#pragma HLS ARRAY_PARTITION variable=output.vals complete
	...
	for (int tt=0; tt<ST; tt++) {
		...
		for (int xx0=0; xx0<S0; xx0++) {
			bool long_burst = (tt==ST-1 || xx0>=S0-2) ? true : false;
			...
			for (int i=0; i<len; i++) {
#pragma HLS PIPELINE II=1
				if (i%2 == 0) output = outputs.read();
				addr[i] = output.vals[i%2];
			}
...
#pragma HLS ARRAY_PARTITION variable=d complete
	...
	result.vals[0] = COMPUTE(d[1], d[3], d[4], d[5], d[7]);
	result.vals[1] = COMPUTE(d[0], d[2], d[3], d[4], d[6]);
...
#pragma HLS PIPELINE II=1
        ...
        bool output_result = (tt==ST-1 || xx0>=S0-2 || xx1==S1/2-1);
