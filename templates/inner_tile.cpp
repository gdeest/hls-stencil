...
void read_tile(data_t *arr, fifo_t &inputs, int T, int N0, int N1, int t, int x0, int x1) {
  ...
  data_t *curr_timestep = &AT_TILED(arr, T, N0, N1, t, x0, x1, -1, -{{depths[1]}}, -{{depths[2]}});
...
#pragma HLS ARRAY_PARTITION variable=input.vals complete
  ...
  for (int tt=-1; tt<ST-1; tt++) {
    ...
    for (int xx0=-2; xx0<S0; xx0++) {
      int len = (tt==-1 || xx0<0) ? S1+{{depths[2]}} : {{depths[2]}};

      for (int i=0; i<len; i++) {
#pragma HLS PIPELINE II=1
        int j = (i+{{depths[2] % unroll[1]}})%{{unroll[1]}};
        input.vals[j] = curr[i];
        if (j=={{unroll[1]-1}}) inputs.write(input);
      }
...
#pragma HLS ARRAY_PARTITION variable=output.vals complete
  ...
  for (int tt=0; tt<ST; tt++) {
    ...
    for (int xx0=0; xx0<S0; xx0++) {
      bool long_burst = (tt==ST-1 || xx0>=S0-{{depths[1]}});
      int len = long_burst ? S1 : {{depths[2]}};
      int pack_nums = long_burst ? S1/{{unroll[1]}} : CEILING({{depths[2]}}, {{unroll[1]}});
      int offset = long_burst ? 0 : {{depths[2]}} % {{unroll[1]}};
      data_t *addr = curr - len;


      for (int i=0; i<len; i++) {
#pragma HLS PIPELINE II=1
        int j = (i + offset) % {{unroll[1]}};
...
#pragma HLS INLINE
  ...
  pack_t buff[2][S0/{{unroll[0]}}][S1/{{unroll[1]}}];
#pragma HLS RESOURCE variable=buff core=RAM_2P
#pragma HLS DATA_PACK variable=buff

  pack_t lines[{{winsize[0]-1}}][(S1/{{unroll[1]}})+{{uhalo[1]}}];
...
#pragma HLS ARRAY_PARTITION variable=win complete dim=0
  ...
  for (int tt=0; tt<ST; tt++) {
    for (int xx0=-{{uhalo[0]}}; xx0<S0/{{unroll[0]}}; xx0++) {
      for (int xx1=-{{uhalo[1]}}; xx1<S1/{{unroll[1]}}; xx1++) {
#pragma HLS PIPELINE II=1
        bool outside = xx0<0 || xx1<0;
        bool read_fifo = outside || tt==0;
        bool output_result = (tt==ST-1 || xx0>=S0/{{unroll[0]}}-{{uhalo[0]}} || xx1==S1/{{unroll[1]}}-{{uhalo[1]}});
...
#pragma HLS stream variable=fi_ctrl2write depth=10
  ...
  #pragma HLS STREAM variable=inputs depth={{(512/unroll[1]) | ceil}}
  #pragma HLS STREAM variable=outputs depth={{(512/unroll[1]) | ceil}}
