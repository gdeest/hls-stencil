...
#define ANISOTROPIC_V1
// #define ANISOTROPIC_V2
...
data_t compute_coeff(data_t nabla, data_t kappa) {
#pragma HLS INLINE
  data_t tmp = nabla / kappa;
  tmp = tmp * tmp;

  #ifdef ANISOTROPIC_V1
  data_t ret = std::exp(-tmp);
// #pragma HLS RESOURCE variable=ret core=FExp_fulldsp
  #else
  data_t ret = 1.0f / (1.0f + tmp);
  #endif
...
#pragma HLS INLINE
  ...
  data_t sqrt2 = 1.141424f;
  data_t dxsqr_inv = 1.0f;
  data_t dysqr_inv = 1.0f;
  data_t ddsqr_inv = 0.50f;
  data_t dt = 0.01f;
  ...
  data_t cN = compute_coeff(nablaN, kappa);
  data_t cS = compute_coeff(nablaS, kappa);
  data_t cE = compute_coeff(nablaE, kappa);
  data_t cW = compute_coeff(nablaW, kappa);
  data_t cNE = compute_coeff(nablaNE, kappa);
  data_t cSE = compute_coeff(nablaSE, kappa);
  data_t cNW = compute_coeff(nablaNW, kappa);
  data_t cSW = compute_coeff(nablaSW, kappa);

  data_t ret = X + (dysqr_inv*(cN*nablaN + cS*nablaS) +
                    dxsqr_inv*(cW*nablaW + cE*nablaE) +
                    ddsqr_inv*(cNW*nablaNW + cSW*nablaSW + cSE*nablaSE + cNE*nablaNE));
