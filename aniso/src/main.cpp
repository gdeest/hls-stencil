...
// #ifdef SW
  ...
  inner_tiles(&dummy0, arr->B0, arr->B0, arr->T, arr->N, ntiles, coords);

// #pragma SDS async(0)
//   inner_tiles(&dummy0, arr->B0,
//               arr->B0,
//               arr->T,
//               arr->N,
//               ntiles/2,
...
// #pragma SDS wait(1) 
  ...
  sem_wait(mutex_cout);
  cout << "WAVEFRONT CYCLES: " << dec << ntiles << "," << (after-before) << endl;
  sem_post(mutex_cout);
#endif
...
void allocate_data_array(data_array *da, int T, int N) {
  ...
  da->N = N;
  int offset = 0;
  da->B0 = alloc_array(B_P0_SIZE(T,N)+B_P1_SIZE(T,N)+B_P2_SIZE(T,N)+B_AUX_SIZE(T,N));
  da->B1 = da->B0 + B_P0_SIZE(T,N);
  da->B2 = da->B1 + B_P1_SIZE(T,N);
  da->Baux = da->B2 + B_P2_SIZE(T,N);
}
...
void verify_output(data_array *da) {
  ...
  cout << "Validating output against golden check..." << endl;
  int count=1;
...
//printf("%d %d %d -> %d %d %d -> %d %d %d\n", p0, p1, p2, st,si,sj,t,i,j);
					...
					// reg[p0][p1][p2] = compute_point(reg[p0-1][p1+1][p+2], reg[p0-1][p1][p2+1], reg[p0-1][p1+1][p2-1], reg[p0-1][p1][p2-2], reg[p0-1][p1-2][p2-1]); 
          reg[p0][p1][p2] = compute_point(reg[p0-1][p1  ][p2  ], reg[p0-1][p1  ][p2-1], reg[p0-1][p1  ][p2-2],
                                          reg[p0-1][p1-1][p2  ], reg[p0-1][p1-1][p2-1], reg[p0-1][p1-1][p2-2],
                                          reg[p0-1][p1-2][p2  ], reg[p0-1][p1-2][p2-1], reg[p0-1][p1-2][p2-2]);
				} else {
