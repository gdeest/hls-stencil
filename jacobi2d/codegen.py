...
if __name__ == '__main__':
    ...
    wrapper_params = ", ".join(["(t)"] + ["(x%d)" % i for i in range(0, ndims)])
    ...
    inner_code = '\n'.join([
        ...
        "    ntiles++; \\",
        "  }",
        "template<> void scan_hw_tiles<%s>(%s, data_array *arr, pthread_barrier_t barriers[2], sem_t *mutex_cout, perf_counter *cnt) {" % (template_params, domain_args),
        inner_code,
        "}\n",
        "#undef INNER_TILE"
    ])
    ...
    outer_code = '\n'.join([
        ...
        "template<> void scan_sw_tiles<%s>(%s, data_array *arr, pthread_barrier_t barriers[2], sem_t *mutex_cout, perf_counter *cnt) {" % (template_params, domain_args),
        outer_code,
        "}\n",
        "#undef OUTER_TILE"
    ])
    ...
    print("#undef max")
    print("#undef min")
    print("#define floord(n,d) (((n)<0) ? -((-(n)+(d)-1)/(d)) : (n)/(d))")
